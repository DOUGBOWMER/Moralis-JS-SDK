...
import { addAddressAptosOperation, AddAddressAptosRequest } from './addAddressAptosOperation';

describe('AddAddressAptosOperation', () => {
  ...
  it('serializeRequest() serializes correctly and deserializeRequest() deserializes correctly with one address', () => {
    ...
    const request: Required<AddAddressAptosRequest> = {
      id: '773c61f9-1ff0-4ab4-b115-2ad8030066df',
      address: AptosAddress.create(address),
    };

    const serializedRequest = addAddressAptosOperation.serializeRequest(request, core);
    ...
    const deserializedRequest = addAddressAptosOperation.deserializeRequest(serializedRequest, core);
